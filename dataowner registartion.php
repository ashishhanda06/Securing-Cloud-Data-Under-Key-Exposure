...
<form action=""  method="GET" style="border:1px solid #ccc">
  <div class="container">
    <h2 style="font-size:50px;">DATA OWNER REGISTRATION</h2>
...
if($_GET['submit'])
{
   $rn = $_GET['Name'];
   $sn = $_GET['email'];
   $cl = $_GET['Mobile Number'];
   $pa = $_GET['Password'];
   $re = $_GET['Repeat Password'];
   $ci = $_GET['City'];


   if($rn!="" && $sn!="" && $cl!="" && $pa!="" && $re!="" && $ci!="")
     {
       $query = "insert into student value('$rn','$sn','$cl','$pa','$re','$ci')";
